...
void Electroniccats_PN7150::PrintBuf(const byte * data, const uint32_t numBytes)
...
void Electroniccats_PN7150::ProcessCardMode(RfIntf_t RfIntf)
{
    ...
    while (rxMessageLength > 0)
    {
        ...
        else if ((rxBuffer[0] == 0x00) && (rxBuffer[1] == 0x00))
        {
            ...
            PrintBuf(&rxBuffer[3],rxBuffer[2]);

            T4T_NDEF_EMU_Next(&rxBuffer[3], rxBuffer[2], &Cmd[3], (unsigned short *)&CmdSize);

            Serial.print("return buffer is : ");
            PrintBuf(&Cmd[3],CmdSize);
...
void NdefPull_Cb(unsigned char *pNdefMessage, unsigned int NdefMessageSize)
{
    Serial.println("in call back func.");
    ...
    while (pNdefRecord != NULL)
    {
        ...
        Serial.println(NdefRecord.recordType);
        ...
        case WELL_KNOWN_SIMPLE_TEXT:
        {
            ...
            if(NdefRecord.recordPayload[0] == 0x02)
            {
                PrintBuf1(&NdefRecord.recordPayload[3],(NdefRecord.recordPayloadSize-3));
            }
            else if(NdefRecord.recordPayload[0] == 0x82)
            {
                PrintBuf1(&NdefRecord.recordPayload[5],(NdefRecord.recordPayloadSize-5));
            }
            NdefRecord.recordPayload[NdefRecord.recordPayloadSize] = save;
        }
        break;
        ...
        case WELL_KNOWN_SIMPLE_URI:
        {
            ...
            PrintBuf1(&NdefRecord.recordPayload[1],NdefRecord.recordPayloadSize);
